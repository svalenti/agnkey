...
#    sys.path.append('/dark/hal/anaconda2/envs/halenv/lib/python2.7/site-packages/')
    location='SV'
else:
    location='deneb'
...
if form.getlist('site'): _site=form.getlist('site')[0]
    ...
    ##########  cadence  #########
if form.getlist('cadence'): _cadence = form.getlist('cadence')[0]
else:                        _cadence = ''
    ##########   vector  #########
...
if form.getlist('airmass'): _airmass=form.getlist('airmass')[0]
    ...
    ##########   obsmode  #########
if form.getlist('obsmode'): _obsmode = form.getlist('obsmode')[0]
else:                        _obsmode = ''
    ##########   namefile  #########
...
if form.getlist('slit'): _slit=form.getlist('slit')[0]
    ...
    ##########   mode  #########
if form.getlist('acmode'): _acmode=form.getlist('acmode')[0]
else:                        _acmode=''
    ##########   filt,exp,nexp  #########
filtvec=[]
expvec=[]
nexpvec=[]
for jj in 'UBVRIugrizY':
    if form.getlist(jj):
...
if _type=='add':
    if not _note:
        ...
        agn_function.insert_values(conn,_table,dictionary)
    ...
    if not _id:
        ...
        _table='noteobjects'
        agn_function.deleteredufromarchive(_id,_table,'id')
     ...
     all1=agn_function.getfromdataraw(conn, _table, 'id', _id,column2='*')
     if len(all1)==1:
         _file=re.sub(workingdirectory,'../',all1[0]['directory']+all1[0]['namefile'])
         os.system('rm '+_file)
         agn_function.deleteredufromarchive(_id,_table,'id')
    ...
    if (int(gg[0]['groupname']) & 1023):
        if SN and SN_RA and SN_DEC:
            ...
            agn_function.insert_values(conn,_table,dictionary1)
            bb = agn_function.getfromcoordinate(conn, 'lsc_sn_pos', SN_RA, SN_DEC,.01056)
            agn_function.updatevalue('lsc_sn_pos','targid',bb[0]['id'],SN,connection='agnkey',namefile0='name')
            dictionary={'name':SN,'targid':bb[0]['id']}
            _JDn=ephem.julian_date()-2400000
            dictionary1={'targid':bb[0]['id'],'groupname':1023,'jd':_JDn}          
            agn_function.insert_values(conn,'recobjects',dictionary)
            agn_function.insert_values(conn,'permissionlog',dictionary1)
     ...
     if 'public' in form.keys():
         ...
         for i in position2.keys(): 
             if i in form.keys():
                 if i !='public':
                     aaa=aaa+position2[i]     
     agn_function.updatevalue('permissionlog','groupname',int(aaa),_id,connection='agnkey',namefile0='id')
    ...
    agn_function.updatevalue('lsc_sn_pos',_key,_note,_id,connection='agnkey',namefile0='id')
...
elif _type=='stopobservation':
    agn_function.updatevalue('triggers',_key,int(_note),_id,connection='agnkey',namefile0='id')
    ...
    proposals=agn_function.readpass['proposal']
    users=agn_function.readpass['users']
    if not _proposal:   
        _proposal=proposals[0]
    dictionary={'user':_user,'targid':int(_targid),'triggerjd':float(_jd), 'filters':','.join(filtvec),'exptime':','.join(expvec),'numexp':','.join(nexpvec),'proposal':_proposal,\
                'site':_site,'instrument':_instrument,'active':1,'cadence':float(_cadence),'mode':_obsmode}
    if _airmass:
        dictionary['airmass'] = float(_airmass)
    agn_function.insert_values(conn,'triggers',dictionary)

elif _type=='triggerfloydscadence':
    print _ntriggers
    print _instrument
    print _expfloyds
    print _nexpfloyds
    ...
    _filters='floyds'
    _instrument='2m0'
    dictionary={'user':_user,'targid':int(_targid),'triggerjd':float(_jd),'cadence':float(_cadence),'active':1,'acqmode':_acmode,\
                'filters':_filters,'exptime':_expfloyds,'numexp':_nexpfloyds, 'slit':_slit,'proposal':_proposal, 'site':_site,'instrument':_instrument, 'airmass':float(_airmass), 'mode':_obsmode}
    agn_function.insert_values(conn,'triggers',dictionary)    
    ...
    passwd = agn_function.readpass['odinpasswd']
    token = agn_function.readpass['token']
    _lunar  = 20
    ...
    for mm in range(0,len(utstart)):
        ...
        logfile,pp = agn_function.sendtrigger_api3(str(SN),str(SN_RA),str(SN_DEC),expvec,nexpvec,filtvec,str(utstart[mm]),str(utend[mm]),_user0,token,_proposal,_instrument,_airmass,_lunar,_site)
        ...
        agn_function.insert_values(conn,'obslog',dictionary)
...
#
    for mm in range(0,len(utstart)):
        logfile,pp = agn_function.sendfloydstrigger_api3(str(SN),_expfloyds,str(SN_RA),str(SN_DEC),str(utstart[mm]),
                                                    str(utend[mm]),_user0,token,_proposal,str(_airmass),_site,_slit,'after', _nexpfloyds )
...
#
    ...
    if fileitem.filename:
            ...
            _note=agn_function.uploadspectrum('../tmp/'+fileitem.filename,_output,_force,_access,_filetype)
            print _note
            _note=re.sub('\n','<br><br>',_note)
    else:  
