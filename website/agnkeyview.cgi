...
hostname, username, passwd, database = agn_function.getconnection('agnkey')
conn = agn_function.dbConnect(hostname, username, passwd, database)
...
def JDnow(verbose=False):
    import datetime
    _JD0=2455927.5
    _JDtoday=_JD0+(datetime.datetime.now()-datetime.datetime(2012, 01, 01,00,00,00)).seconds/(3600.*24)+\
...
def plotfast(setup,output='',logs=''):#,band,color,fissa=''):
    ...
    global idd,_mjd,_mag,_setup,_namefile,shift
    ...
    for _tel in _setup:
        ...
        for _fil in _setup[_tel]:
            shift=_shift[_fil]
            col=_color[_fil]
            plt.plot(array(_setup[_tel][_fil]['mjd']),array(_setup[_tel][_fil]['mag'])+shift,_symbol[ii],color=col,label=_tel+' '+_fil+' '+str(shift),markersize=5)
    ...
    _JDn=JDnow()
    if logs:
       sta=array(logs[0],float)
       sto=array(logs[1],float)
...
def run_getmag(imglist,_field,_output='',_bin=1e-10,magtype='mag',_ft=1,logs=''):
     ...
     if _field=='landolt': 
         filters0 = ['U','B','V','R','I','Bessell-B','Bessell-V','Bessell-R','Bessell-I'] # to be raplace when more telescopes available with dictionary
     else:                 
     ...
     for _tel in setup:
        ...
        for _fil in setup[_tel]:
            mjd00=compress((array(filt)==_fil)&(array(tel)==_tel)&(array(filetype)==_ft),array(mjd))
            ...
            mag1,dmag1,mjd1,date1,namefile1=[],[],[],[],[]
            done=[]
            for i in range(0,len(mjd0)):
                if i not in done:
                    ww=asarray([j for j in range(len(mjd0)) if (mjd0[j]-mjd0[i])<_bin and (mjd0[j]-mjd0[i])>=0.0]) # abs(mjd0[j]-mjd0[i])<bin])
...
if not _targid:       #  try to get coordinate from NED
  if not SN_RA and not SN_DEC:     #  try to get coordinate from name
    if SN0:
        listac=agn_function.getlike(conn, 'recobjects', 'name',SN0,'targid,name')
        allobj=''
        alltarg=[]
        for i in listac: 
        ...
        if len(alltarg)==0:
           ...
           lista2=agn_function.getfromdataraw(conn,'lsc_sn_pos','targid',str(int(listac[0]['targid'])),'ra_sn,dec_sn,targid,name')
...
if (SN_RA and SN_DEC) and not _targid:     #  try to get name from coordinate
    ...
    aa=agn_function.getfromcoordinate(conn, 'lsc_sn_pos', SN_RA, SN_DEC,.01)
    if len(aa)>=1:
...
print ''' </head>'''
print '''<script> $(function() {$("#includedContent2").load("./schedule2.cgi?targid=%s"); }); </script>''' % (_targid)
print ''' </head>'''
...
commandg = ["select groupname from userstab where user='"+str(_user)+"'"]
gg = agn_function.query(commandg)
if len(gg)==0:
    print '<h3> sorry user unknown, you dont have access to this page <h3>'
    print '''</div>'''
    print '</body>'
...
#    commandpermission=['select * from permissionlog where targid='+str(_targid)+' and (groupname & '+str(num)+')='+str(num)+'']
    listaper=agn_function.query(commandpermission)
    if not listaper:
        print gg
...
if _targid:
    ...
    lista9=agn_function.query(command9)
else:
...
#######################################################################
if _targid:
    command=["select * from dataredulco as a where a.targid="+str(_targid)+" order by a.mjd desc"]
    lista=agn_function.query(command)
else:
    if SN_RA and SN_DEC:       # photometry 
        distance=1
        command=["set @sc = pi()/180"," set @ra = "+str(SN_RA),"set @dec = "+str(SN_DEC),"set @distance = "+str(distance),"SELECT *,abs(2*asin( sqrt( sin((a.dec0-@dec)*@sc/2)*sin((a.dec0-@dec)*@sc/2) + cos(a.dec0*@sc)*cos(@dec*@sc)*sin((a.ra0-@ra)*@sc/2)*sin((a.ra0-@ra)*@sc/2.0) )))*180/pi() as hsine FROM dataredulco as a HAVING hsine<@distance order by a.mjd desc"]
        lista=agn_function.query(command)
    elif  SN0:
        command=["select * from dataredulco as a where a.dateobs <="+"'"+d+"' and a.dateobs >="+"'20120909' and a.objname like '%"+SN0+"%'  order by a.mjd desc "]
        lista=agn_function.query(command)
    else:
        lista=''
if len(lista):
    ...
    wwimages=asarray([j for j in range(len(ll0['mjd'])) if (float(_JDn)-ll0['mjd'][j]< 7)])
else:
    wwsloan=[]
    wwlandolt=[]
    wwimages=[]
...
if len(wwimages)>0:
   listpng=array(['../../AGNKEY/'+re.sub(workingdirectory,'',k)+re.sub('.fits','.png',v) for k,v in  zip(ll0['wdirectory'],ll0['namefile'])])[wwimages]
else:  
    listpng=[]

if len(lista):
    llimage=['wget --user='+str(_user)+' --password=xxxx '+'http://dark.physics.ucdavis.edu/~hal/AGNKEY/'+re.sub(workingdirectory,'',k)+v for k,v in  zip(ll0['wdirectory'],ll0['namefile'])]
else: llimage=[]
...
if _targid:
    ...
    lista33e=agn_function.query(command1e)
else:
...
if ll22e:
   for i in range(0,len(ll22e['objname'])):
      ...
      if location=='deneb':
          directory=re.sub(workingdirectory,'http://'+base_url+'/AGNKEY/',ll22e['directory'][i])
          directory2='../AGNKEY/'+re.sub('/home/cv21/AGNKEY_www/AGNKEY/','',ll22e['directory'][i])
      elif location=='phys-dark':
          directory=re.sub(workingdirectory,'http://dark.physics.ucdavis.edu/~hal/AGNKEY/',ll22e['directory'][i])
          directory2='../../AGNKEY/'+re.sub(workingdirectory,'',ll22e['directory'][i])
      else:
      ...
      if _user in agn_function.readpass['superusers']:
         deletespectrum=agn_function.delspectrum(SN0,SN_RA,SN_DEC,_targid,ll22e['id'][i],ll22e['namefile'][i],_user,'agnkeyview.cgi')
      else:
         deletespectrum=''
...
if _targid:
    ...
    lista3=agn_function.query(command1)
else:
    if SN_RA and SN_DEC:       # spectra
        distance=1
...
if ll2:
    ...
    for i in range(0,len(ll2['objname'])):
        ...
        note[jjd]=ll2['note'][i]
        imm[jjd]='../../AGNKEY/'+re.sub(workingdirectory,'',ll2['directory'][i])+ll2['namefile'][i]
#        imm[jjd]='../../AGNKEY/'+re.sub(agnkey.util.workingdirectory,'',ll22e['directory']
    ...
    for jjd in sort(riga.keys()):
      ...
      jjj=agn_function.grafico1(nnn,'25','150')+'</p>'
      #else:
      #    jjj='</p>'
...
bb=''
if _targid:
    lista2=agn_function.getfromdataraw(conn,'obslog','targid',str(_targid),'windowstart,windowend')
    if len(lista2)>0:
        log1={}
...
if len(wwsloan)>0:
    ...
    html=run_getmag(ll1,'sloan','../../tmp/'+SN0+'_sloan1_'+_user,1e-10,'ph1',1,bb)
    if html:
       nameimage=html
    else:
       nameimage=''
    graph1=agn_function.grafico(nameimage,'30','30')
else: graph1=agn_function.grafico('')
...
if len(wwsloan)>0:
    ...
    if html:
       ...
       nameimage=''
    graph11=agn_function.grafico(nameimage)
else: graph11=agn_function.grafico('')
...
if len(wwsloan)>0:
    ...
    if html:
       ...
       nameimage=''
    graph2=agn_function.grafico(nameimage)
else: graph2=agn_function.grafico('')
...
if len(wwsloan)>0:
    ...
    if html:
       ...
       nameimage=''
    graph22=agn_function.grafico(nameimage)
else: graph22=agn_function.grafico('')


##################################
if _targid:
    command1=["select * from noteobjects where targid="+str(_targid)+" order by id "]
    lista44=agn_function.query(command1)
else: lista44=''

note=''
if len(lista44):
   for ii in lista44:
      note=note+str(ii['datenote'])+'&#09;'+'&bull;'+'&#09;'+str(ii['user'])+'&#09;'+'&bull;'+'&#09;'+str(ii['note'])+agn_function.delfromdb(SN0,SN_RA,SN_DEC,_targid,ii,_user,'agnkeyview.cgi')
...
if len(listpng)>0:
   for img in listpng:
      graph=agn_function.grafico1(img)
      pngfiles=pngfiles+'<tr>'+tdopen+graph+tdclose+'</tr>'

if (SN_RA and SN_DEC):
    html2=agn_function.visibility(SN_RA,SN_DEC,True,'500','300')
    graph10=agn_function.grafico(html2,'100','100')
else:
  graph10=''
#############################################################      
aka=''
if _targid:
  ...
  listar=agn_function.query(commandr)
  if len(listar)>1:
    for hh in listar:
...
if _targid:
  ...
  listar1=agn_function.query(commandr1)
else:
  listar1=''
...
print '<tr align="center" BGCOLOR="#F5F5DC"> <td width=100 > <h3> '+str('AGN NAME')+'  </h3></td><td width=100> <h3> Redshift </h3></td>'+\
...
if listar1:
  if _user in agn_function.readpass['superusers']:
    print '<tr BGCOLOR="#F5F5DC" align="center"><td > <h3>'+str(SN0)+ '</h3> </td> <td width=300> <h3>'+str(listar1[0]['redshift'])+'</h3> </td>'+\
          ...
          ' <td align="center" width=100> <h3> '+aka+' </h3> </td></tr>'
    print '<tr BGCOLOR="#F5F5DC" align="center"><td >'+'</td> <td width=300>'+agn_function.objectinfo(listar1[0],_user,'redshift','agnkeyview.cgi')+\
          '</td> <td width=300>'+agn_function.objectinfo(listar1[0],_user,'ra_sn','agnkeyview.cgi')+\
          '</td> <td width=300>'+agn_function.objectinfo(listar1[0],_user,'dec_sn','agnkeyview.cgi')+\
          '</td> <td width=300>'+agn_function.objectinfo(listar1[0],_user,'objtype','agnkeyview.cgi')+\
          '</td> <td width=300>'+agn_function.recinfo(listar1[0],_user,'agnkeyview.cgi')+'</td>'+\
          '</tr>'
  else:
    print '<tr BGCOLOR="#CCFF66" align="center"><td > <h3>'+str(SN0)+ '</h3> </td> <td width=100> <h3>'+str(listar1[0]['redshift'])+'</h3> </td>'+\
          ...
          ' <td width=100 ><h3>'+'</h3></td width=300> <td> <h3> '+aka+' </h3> </td></tr>'   
print """</table>"""
...
print '</td>'+'<td width=50>'
if listar1:
    print '<td width="200">'+'''<IMG SRC="http://skyservice.pha.jhu.edu/DR8/ImgCutout/getjpeg.aspx?ra='''+str(listar1[0]['ra_sn'])+'''&dec='''+str(listar1[0]['dec_sn'])+'''&scale=0.79224   &width=350&height=350&opt=GST&query=SR(10,20)">'''+'</td>'
print '</td>'+'</tr>'
print """</table>"""
#print '<br></br>'

print '<a id="notes"></a>'
print """<table class='fixed' style='table-layout:fixed'  border='0' BGCOLOR='#F5F5DC'  height='10%' cellspacing='0' cellpadding='0' width='800' >"""
print " <td align='center' width='700'> <h3> notes </h3> <td>"""
if note:   print "<tr><td width='800'>"+note+"</td></tr>"
else:      print "<tr><td width='800%'></td></tr>"
print """</table>"""
print '<br></br>'
_note=agn_function.addnote(_targid,SN0,SN_RA,SN_DEC,_user,'agnkeyview.cgi')
print _note
...
#print '<br> ____________________________________________________________________________________ </br>'
print agn_function.triggercadence(str(SN0),str(SN_RA),str(SN_DEC),str(_targid),'agnkeyview.cgi',{},agn_function.readpass['proposal'])
print '<br> ____________________________________________________________________________________ </br>'
print agn_function.triggerfloydscadence(str(SN0),str(SN_RA),str(SN_DEC),str(_targid),'agnkeyview.cgi',{},agn_function.readpass['proposal'])
print '<br> ____________________________________________________________________________________ </br>'
commandtrigger = ['select * from triggers where active = 1 and targid = '+str(_targid)+' ']
listatr=agn_function.query(commandtrigger)

if len(listatr):
    lltr={}
    for jj in listatr[0].keys(): 
        lltr[jj] = []
    for i in range(0,len(listatr)):
        for jj in listatr[0].keys(): 
            lltr[jj].append(listatr[i][jj])
else: 
    lltr = ''
if lltr:
    print '<h4>  ACTIVE TRIGGERS  </h4>'
    print '<table  border="1">'
    line0=''
    for key in ['cadence','instrument','filters','exptime','mode','site','slit','proposal']:
      line0=line0+'<td>'+str(key)+'</td>'
    print '<tr>'+line0+'</tr>'
    for i in range(0,len(lltr['exptime'])):
        line = ''
        for key in ['cadence','instrument','filters','exptime','mode','site','slit','proposal']:
            line = line+'<td>'+str(lltr[key][i])+'</td>'#+'<br>'+agnkey.agndefin.objectinfo(lltr[i],_user,key,'agnkeyview.cgi')+'</td>'
        line = line + '<td> '+ agn_function.stopobservation(str(lltr['id'][i]),str(lltr['targid'][i]),_user,'active','agnkeyview.cgi')+'</td>'
        print '<tr>'+line+'</tr>'
    print '</table>'
else:
    print '<h4>  NO ACTIVE TRIGGERS  </h4>'
...
print '<tr align="center" > <td> psf mag  ',agn_function.plot_phot(conn,_targid, width=400, height=250, plottype='flot',magtype='mag'),\
    '</td><td> aperture mag 1 ', agn_function.plot_phot(conn,_targid, width=400, height=250, plottype='flot',magtype='appmagap1'),'</td> </tr>'
print '<tr align="center" ><td>  aperture mag2 ',agn_function.plot_phot(conn,_targid, width=400, height=250, plottype='flot',magtype='appmagap2'),\
    ' </td><td> aperture mag 3', agn_function.plot_phot(conn,_targid, width=400, height=250, plottype='flot',magtype='appmagap3'),'</td></tr>'
print '</table>'

print '<div id="includedContent"></div>'
print '<div id="includedContent2"></div></div>'
