...
commandtrigger = ['select t.id, t.cadence,t.filters,t.targid, t.exptime,t.site,t.slit,t.proposal,t.instrument, '+\
                  'l.name, l.objtype '+\
                  ' from triggers as t join lsc_sn_pos as l where t.targid = l.id and t.active = 1 order by t.targid']
...
if len(bb)>0:
    if int(bb[0]['groupname'])==1:
        if lltr:
            ...
            line0=''
            for key in ['name','objtype','cadence','instrument','filters','exptime','site','slit','proposal']:
            ...
            for i in range(0,len(lltr['exptime'])):
                ...
                for key in ['name','objtype','cadence','instrument','filters','exptime','site','slit','proposal']:
                    if key =='name':
                        line = line + '<td><a href="agnkeyview.cgi?sn_name='+re.sub('\+','%2B',str(lltr[key][i]))+'"> '+str(lltr[key][i])+' </a></td>'
                    else:
                        line = line + '<td>'+str(lltr[key][i])+'</td>'
                commandnew=['select windowstart,windowend,status from triggerslog where triggerid='+str(str(lltr['id'][i]))+' order by id DESC limit 1']
                listatr1=agn_function.query(commandnew)
                if listatr1:
                    _date0 = agn_function.jd2date(listatr1[0]['windowstart'])
                    _date1 = agn_function.jd2date(listatr1[0]['windowend'])
                    line = line+'<td>'+\
                    '<table><tr><td>'+str(_date0)+'</td></tr>'+\
                    '<tr><td>'+str(_date1)+'</td></tr>'+\
                    '<tr><td>'+str(listatr1[0]['status'])+'</td></tr></table>'+\
                    '</td>'
