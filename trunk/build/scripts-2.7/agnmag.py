...
import time, math
import agnkey
from numpy import argmin, array, isfinite,compress,sqrt
      ...
      for _filter in dicti:
        for img in dicti[_filter]:
            ...
            if len(secondimage)>0:
                ...
                dicti[_filter][img]['secondimg']=secondimage[argmin(jdvec)]
                dicti[_filter][img]['secondfilt']=filtvec[argmin(jdvec)]
...
#                mag0=dicti[_filter][img][namemag[_typemag][0]]+2.5*math.log10(dicti[_filter][img]['exptime'])-kk[filters1[_filter]]*dicti[_filter][img]['airmass']
                ...
                if dicti[_filter2][img2]['telescope'] in ['1m0-04','1m0-05','1m0-06','1m0-09']:    kk=agnkey.sites.extintion('ctio')
                elif dicti[_filter2][img2]['telescope'] in ['elp','1m0-08']:                       kk=agnkey.sites.extintion('mcdonald')
                elif dicti[_filter2][img2]['telescope'] in ['cpt','1m0-12','1m0-10','1m0-13']:     kk=agnkey.sites.extintion('southafrica')
                elif dicti[_filter][img]['telescope'] in ['ftn']:                                  kk=agnkey.sites.extintion('mauna')
...
#                mag1=dicti[_filter2][img2][namemag[_typemag][0]]+2.5*math.log10(dicti[_filter2][img2]['exptime'])-kk[filters1[_filter2]]*dicti[_filter2][img2]['airmass']
                ...
                if filters1[_filter].upper()==col[0]:
                    ...
                    DM11=sqrt((dm1*dmag0)**2+(dz1*DZ1)**2+(dm2*dmag1)**2+(dz2*DZ2)**2)
                    ...
                    if _interactive:
                        ...
                        print M1,M2
                    try:
                        if isfinite(M1) and  M1 < 999:
                            agnkey.agnsqldef.updatevalue(_datatable,'mag',M1,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                            if  _typemag=='fit':       agnkey.agnsqldef.updatevalue(_datatable,'magtype',2,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                            elif _typemag=='ph':       agnkey.agnsqldef.updatevalue(_datatable,'magtype',3,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                            agnkey.util.updateheader(img,0,{'mag':[M1,'calibrated mag']})
                        else:       
                            agnkey.agnsqldef.updatevalue(_datatable,'mag',9999,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                            agnkey.util.updateheader(img,0,{'mag':[9999,'calibrated mag']})
                        if isfinite(DM11):    
                            agnkey.agnsqldef.updatevalue(_datatable,'dmag',DM11,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                            agnkey.util.updateheader(img,0,{'dmag':[DM11,'calibrated mag error']})
                        else:       
                            agnkey.agnsqldef.updatevalue(_datatable,'dmag',9999,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                    ...
                    DM22=sqrt((dm1*dmag0)**2+(dz1*DZ1)**2+(dm2*dmag1)**2+(dz2*DZ2)**2)
                    ...
                    if _interactive:
                        ...
                        print M1,M2
                    try:
                        if isfinite(M2) and  M2 < 999:
                            agnkey.agnsqldef.updatevalue(_datatable,'mag',M2,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                            if  _typemag=='fit':       agnkey.agnsqldef.updatevalue(_datatable,'magtype',2,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                            elif _typemag=='ph':       agnkey.agnsqldef.updatevalue(_datatable,'magtype',3,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                            ...
                            agnkey.util.updateheader(img,0,{'mag':[9999,'calibrated mag']})
                        if isfinite(DM22):    
                            agnkey.agnsqldef.updatevalue(_datatable,'dmag',DM22,re.sub('sn2.fits','fits',string.split(img,'/')[-1]))
                ...
                for ww in  dicti[_filter][img].keys():
                    if 'ZP'+filters1[_filter].upper()==ww[0:3] and float(string.split(dicti[_filter][img][ww])[1])<99:
