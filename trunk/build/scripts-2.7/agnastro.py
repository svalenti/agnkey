...
from agnkey.util import readkey3, readhdr, delete, updateheader
...
if __name__ == "__main__":
     ...
     if _catalogue in ['inst']:  _catalogue2=['2mass','usnoa2','usnob1']
     #elif _catalogue in ['2mass','usnoa2','usnob1']:   _catalogue2=_catalogue
     else:   _catalogue2=[_catalogue]
     for img in imglist:
       hdr=readhdr(img)
       _wcserr=readkey3(hdr,'wcserr')
       _astromet=readkey3(hdr,'ASTROMET')
       done=0
       ...
       if done==1 and _redo==False:  print '\n#####  astrometry already done'
         ...
         try:
            for cat in _catalogue2: #['2mass','usnoa2']:#,'usnob1']:
               rmsx3,rmsy3,num3,fwhmgess,ellgess,ccc,rasys3,decsys3,mbkg3=agnkey.agnastrodef.agnastroloop([img],cat,_interactive,number1,number2,number3,'rxyscale',_t1,_t2,sexvec,True,_minnum,_method,_xshift,_yshift)
               if rmsx3 <= 2 and rmsy3 <= 2: break
            print 'no good solution with '+str(_minnum)+' '+str(number1)
            if rmsx3 > 2 and rmsy3 > 2:
               for cat in _catalogue2: #['2mass','usnoa2']:#,'usnob1']:
                   rmsx3,rmsy3,num3,fwhmgess,ellgess,ccc,rasys3,decsys3,mbkg3=agnkey.agnastrodef.agnastroloop([img],cat,_interactive,int(number1/2.),int(number2/2.),\
               ...
               if rmsx3 > 2 and rmsy3 > 2:
                   for cat in _catalogue2: #['2mass','usnoa2']:#,'usnob1']:
                       rmsx3,rmsy3,num3,fwhmgess,ellgess,ccc,rasys3,decsys3,mbkg3=agnkey.agnastrodef.agnastroloop([img],cat,_interactive,int(10),int(10),\
         ...
         if fwhmgess and fwhmgess < 99: 
            ...
            agnkey.util.updateheader(img,0,{'PSF_FWHM':[fwhmgess,'FHWM (arcsec) - computed with sectractor'],'ELLIPTIC':[ellgess,'ellipticity of point sources (1-b/a)'],\
       ...
       try: 
           ...
           print _astromet
           if _astromet:
               if float(string.split(_astromet)[0])<2 and float(string.split(_astromet)[1])<2:       WCSERR=0
               else:      WCSERR=99
           else:      WCSERR=99
           print WCSERR
           if WCSERR==0: 
