...
if __name__ == "__main__":
     ...
     parser.add_option("-n", "--iteration",dest="niter",default=3,
           type='int',help=' number of iterations \t\t %default')
     parser.add_option("-x", "--xorder",dest="xorder",default=2,
           type='int',help=' order for backgorund in x \t\t %default')
     parser.add_option("-y", "--yorder",dest="yorder",default=2,
           type='int',help=' order for backgroun in y \t\t %default')
     parser.add_option("-b", "--bkg",dest="bkg",default=4,
           type='float',help=' backgroun radius dimension \t\t %default')
     parser.add_option("-z", "--size",dest="size",default=7,
           type='float',help=' half size of the stamp \t\t %default')
     parser.add_option("-m", "--datamax",dest="datamax",default=51000,
           type='float',help=' data max for saturation \t\t %default')
     parser.add_option("-i", "--interactive",action="store_true",\
            dest='interactive',default=False, help='Interactive \t\t\t [%default]')
     parser.add_option("-s", "--show",action="store_true",\
            dest='show',default=False, help='display images \t\t\t [%default]')
     ...
     if option.psf:   psflist=agnkey.util.readlist(option.psf)
     else:            psflist=''
     _ra=option.RA
     _dec=option.DEC
     _size=option.size
     ...
     if option.show==False:          _show=False
     else:                         _show=True
     if option.redo==False:      redo=False
     else:                       redo=True
     arterr=''
...
######
       try:
            ...
            agnkey.agnsqldef.updatevalue('dataredulco','psfdmag',9999,string.split(img,'/')[-1]+'.fits')
            agnkey.agnsqldef.updatevalue('dataredulco','psfx',9999,string.split(img,'/')[-1]+'.fits')
            agnkey.agnsqldef.updatevalue('dataredulco','psfy',9999,string.split(img,'/')[-1]+'.fits')
...
######
       ...
       print '##########  '+str(filter)+'  ##############'
       if not psflist:  psfimage=img+'.psf'
       else:  psfimage=re.sub('.fits','',psflist[0])
       skip=0
       ...
       else: sys.exit('psf not computed')
       print psfimage
       if not os.path.exists(psfimage+'.fits'):        sys.exit('missing psf file')
            
       if redo:   skip=0

       if skip==0:             
        iraf.set(stdimage='imt2048')
        if _interactive:                _z1,_z2,goon=agnkey.util.display_image(img+'.fits',1,'','',True,_xsize=1,_ysize=1)
        elif _show:                           _z1,_z2,goon=agnkey.util.display_image(img+'.fits',1,'','',False)
        apco0=agnkey.util.readkey3(hdr2,'APCO')

        if 'PIXSCALE' in hdr2: pixelscale= agnkey.util.readkey3(hdr2,'PIXSCALE')
        elif 'CCDSCALE' in hdr2: pixelscale= agnkey.util.readkey3(hdr2,'CCDSCALE')
        ...
        _ra0,_dec0,_SN0=agnkey.util.checksnlist(img+'.fits','supernovaelist.txt')
        if not _ra0 and not _dec0:  _ra0,_dec0,_SN0,_tt=agnkey.util.checksndb(img+'.fits','lsc_sn_pos')

        if _SN0:  print '\n###  SN in our list '+_SN0 
        if not _ra and not _dec:
            _ra=_ra0
            _dec=_dec0
        else: print '\n### use coordinate from user'
        if _ra and _dec:
            os.system('rm -rf tmp.*')
            ff=open('tmp.tv','w')
            ff.write(str(float(_ra))+' '+str(float(_dec)))
            ff.close()
            lll=[str(_ra)+'    '+str(_dec)]
            iraf.wcsctran('STDIN','tmp.pix',img,Stdin=lll,inwcs='world',units='degrees degrees',outwcs='logical',columns='1 2',formats='%10.1f %10.1f')
            if _show:  iraf.tvmark(1,'tmp.pix',mark="circle",number='yes',radii=10,nxoffse=5,nyoffse=5,color=214,txsize=2)
...
######################    POINT TO SN #######################################
        ...
        if not _interactive or _size:
             ...
             agnkey.util.delete("original.fits")    
          ...
          while repeat=='n':            
             ...
             _tmp1,_tmp2,goon=agnkey.util.display_image('original.fits',1,'' ,'' , False, _xsize=.5, _ysize=.5)
             repeat = raw_input('### ok ? [y/n] ? [y] ')
             if not repeat: repeat='y'
             elif repeat=='no': repeat='n'
        if _show:
            _z11,_z22,good=agnkey.util.display_image('original.fits', 1, '', '', False, _xsize=.5, _ysize=.5)       
            z11,z22=_z11,_z22
        if _interactive:
          answ = 'y'
...
#            z22=float(_z22)
        if not _interactive and xx0:
          ...
          answ0='n'
          while answ0=='n':
            _tmp1,_tmp2,goon=agnkey.util.display_image('original.fits',1,z11,z22,False)
            ...
            for i in vector:
                ff.write(i+' \n')
            ff.close()
            if _show:   iraf.tvmark(1,'tmplabel',autol='no',mark="circle", radii=10, inter='no',label='no', number='yes',\
                                         pointsize=20, txsize=2,color =204)
            os.system('cp tmplabel '+img+'.sn.coo')
        ...
        while answ0 == 'n':
	            ...
	            if not _interactive or _fb:
	    	       if _fb: leng0=_fb
                    ...
                    try: 
                        ...
                        checkleng0=='yes'
                if _show:    iraf.tvmark(1,img+".sn.coo",auto='no',mark="rectangle",length=int(fwhm0*float(leng0)),inter='no', color=204) 
            ...
            if _show:
                ...
                _tmp1,_tmp2,goon=agnkey.util.display_image('sky.fits',1, z11, z22, False, _xcen=.25, _ycen=.75, _xsize=.3, _ysize=.3, _erase='no')
            agnkey.util.delete("tmptbl")
            ff=open('tmptbl','w')
            ...
            if _show:
                iraf.tvmark(1,"tmptbl",autol='no',mark="none",inter='no',label='yes',txsize=2)
                _tmp1,_tmp2,goon=agnkey.util.display_image('sn.fits',1, z11, z22, False, _xcen=.75, _ycen=.25, _xsize=.3, _ysize=.3, _erase='no')
            agnkey.util.delete("tmptbl")
            ff=open('tmptbl','w')
...
####################################    FITSN        ###################################
        ...
        apori1,apori2,apori3,apmag1,apmag2,apmag3,fitmag,truemag,magerr,centx,centy=agnkey.agnsnoopy.fitsn(img,psfimage,img+'.sn.coo',_recenter,fwhm0,'original','sn','residual',_show,_interactive,z11,z22,midpt,_size,apco0,_dmax)
#################       Iterate Beckground    ###################################
        if _interactive:
            if not _numiter:
	    ...
	    print '######'
            agnkey.util.delete("sn.fits,residual.fits,snfit.fits,tmp.fits")
            checkorder='yes'
            while checkorder=='yes':
	       if not _interactive or _xord and _yord:
		       if _xord and _yord: 
            ...
            apori1,apori2,apori3,apmag1,apmag2,apmag3,fitmag,truemag,magerr,centx,centy=agnkey.agnsnoopy.fitsn(img,psfimage,\
                         img+'.sn.coo',_recenter,fwhm0,'original','sn','residual',_show,_interactive,z11,z22,midpt,_size,apco0,_dmax)
        ...
        for i in range(len(fitmag)):
            print "SN",i,str(centx[i]+x1-1),str(centy[i]+y1-1),str(centx[i]),str(centy[i]),"  ",str(apori3[i]),"  ",str(apmag3[i]),"  ",str(truemag[i]),"  ",str(arterr),"  ",str(magerr[i])
        print "**************************************************************************"
        ...
        while answ0== 'y':
            ...
            while checkdm=='yes':
                ...
                try: 
                    ...
                    checkdm='yes'
            apori1,apori2,apori3,apmag1,apmag2,apmag3,fitmag,truemag,magerr,centx,centy,newmag=agnkey.agnsnoopy.manusn(img,psfimage,dmag0,apori1,apori2,apori3,apmag1,apmag2,apmag3,fitmag,truemag,magerr,centx,centy,z11,z22,midpt,_size,fwhm0,x1,y1,arterr)
            try:
...
#
        ...
        if answ0=='y':
            leng0=4
            try:
                 _arterr2,_arterr=agnkey.agnsnoopy.errore(img,'artlist.coo',size,truemag,fwhm0,leng0,False,False,_numiter,z11,z22,midpt,nax,nay,xbgord0,ybgord0,_recenter,apco0,_dmax)
            except:
...
#            misu='SN%1.1s %6.6s %6.6s %6.6s %6.6s %6.6s\n' % (str(i+1),str(apori3[i]),str(apmag3[i]),str(truemag[i]),str(arterr),str(magerr[i]))
            if truemag[i]=='INDEF': truemag[i],arterr,magerr[i]=9999,0.0,0.0
            if apmag3[i]=='INDEF': apmag3[i]=9999
            
            headers={'PSFX'+str(i+1):[str(centx[i]+x1-1),'x pos psf mag'],'PSFY'+str(i+1):[str(centy[i]+y1-1),'y pos psf mag'],
                     'PSFMAG'+str(i+1):[str(truemag[i]),'psf magnitude'],'PSFDMAG'+str(i+1):[str(max(arterr,magerr[i])),'psf mag error'],
                     'APMAG'+str(i+1):[str(apmag3[i]),'ap mag after bgsub']}
            agnkey.util.updateheader(img+'.sn2.fits',0,headers)
        agnkey.util.delete("apori")
        agnkey.util.delete("sec")
        agnkey.util.delete("skyfit.fits")
        agnkey.util.delete("sn.fits")
        agnkey.util.delete("artsn.fits")
        agnkey.util.delete("artsky.fits,artbgs.fits")
        agnkey.util.delete("bg.fits,bgs.fits")
        agnkey.util.delete("tmp*")
        agnkey.util.delete(img+".sn.*")
        ...
        try:
             agnkey.agnsqldef.updatevalue('dataredulco','psfmag',truemag[0],string.split(img,'/')[-1]+'.fits')
             agnkey.agnsqldef.updatevalue('dataredulco','psfdmag',max(arterr,magerr[0]),string.split(img,'/')[-1]+'.fits')
             agnkey.agnsqldef.updatevalue('dataredulco','psfx',centx[0]+x1-1,string.split(img,'/')[-1]+'.fits')
             agnkey.agnsqldef.updatevalue('dataredulco','psfy',centy[0]+y1-1,string.split(img,'/')[-1]+'.fits')
             agnkey.agnsqldef.updatevalue('dataredulco','apmag',apmag3[0],string.split(img,'/')[-1]+'.fits')
        except: print 'module mysqldef not found'
