                ...
                if o in listatemp[f]:
                    ...
                    for imgtar in imglist1:
                        ...
                        if not os.path.isfile(_dir + imgout) or _force:
                            ...
                            _afssc = ''
                            if afssc:
                                substamplist, dict = crossmatchtwofiles(imgtar, imgtemp, 3)
                                xra1, xdec1, xra2, xdec2, xpix1, ypix1, xpix2, ypix2 = dict['ra1'], dict['dec1'], \
                                                                                       dict['ra2'], dict['dec2'], \
                                                                                       dict['xpix1'], dict['ypix1'], \
                            ...
                            line = agnkey.util.execdirectory + "hotpants -inim " + \
                                   ...
                                   ' -savexy ' + re.sub('.fits', 'xy', imgout) + ' -oci ' + \
                                   re.sub('.fits', '.conv.fits', imgout)
                            print line
                            os.system(line)
                            hd = pyfits.getheader(imgout)
                            agnkey.util.updateheader(imgout, 0,
                                                     {'template': [string.split(imgtemp0, '/')[-1], 'template image']})
                            agnkey.util.updateheader(imgout, 0,
                                                     {'target': [string.split(imgtar0, '/')[-1], 'target image']})
                            ...
                            if not os.path.isfile(dictionary['wdirectory'] + imgout) or _force in ['yes', True]:
                                ...
                                os.system('mv ' + imgtemp + ' ' + dictionary['wdirectory'] + re.sub('.diff.', '.ref.',
                                                                                                    imgout))
                            ...
                            if os.path.isfile(
                                            pathscale + re.sub('.fits', '.sn2.fits', string.split(imgscale, '/')[-1])):
                                os.system(
                                    'cp ' + pathscale + re.sub('.fits', '.sn2.fits', string.split(imgscale, '/')[-1]) +
                            ...
                            if ggg:
                                agnkey.agnsqldef.deleteredufromarchive(string.split(imgout, '/')[-1],
                                                                       'inoutredu', 'nameout')
                            dictionary = {'namein': string.split(imgtar0, '/')[-1],
                                          'nameout': string.split(imgout, '/')[-1],
