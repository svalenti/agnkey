...
# archicingspectra
#
# create table datareduspectra (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, namefile VARCHAR(50)
# UNIQUE KEY, directory VARCHAR(100), objname VARCHAR(50), jd DOUBLE, dateobs DATE, exptime FLOAT,
# filter VARCHAR(20), grism VARCHAR(20), telescope VARCHAR(20), instrument VARCHAR(20), airmass FLOAT,
# ut TIME, slit VARCHAR(20), status VARCHAR(50), original VARCHAR(50), note VARCHAR(100), ra0 FLOAT,
# dec0 FLOAT, PROPID VARCHAR(30), observer VARCHAR(30), dateobs2  VARCHAR(23), targid INT default 0);
#################################################
...
def archivereducedspectrum(img):
    ...
    if 'DATE-OBS' in hdr:
        ...
        if 'T' in _dateobs:
            _dateobs = string.split(_dateobs, 'T')[0]
    else:
        _dateobs = ''
    #   UT
    if 'UTSTART' in hdr:
    ...
    if _telescope in ['fts', 'ftn']:
        dictionary = {'dateobs': _dateobs, 'exptime': hdr.get('exptime'), 'filter': hdr.get('filter'),
                      ...
                      'PROPID': hdr.get('PROPID'), 'dateobs2': hdr.get('DATE-OBS')}

    elif 'gemini' in _telescope.lower():
        if 'south' in _telescope.lower():
            _telescope = 'gs'
        else:
            _telescope = 'gn'
...
def uploadspectrum(img, _output, _force):
    ...
    if 'gemini' in _tel.lower():
        ...
        try:
            hdr0.__delitem__('AIRMASS')
        except:
            hdr0.remove('AIRMASS')
        hed = ['TELESCOP', 'OBSERVAT', 'RA', 'DEC', 'UT', 'ST', 'EXPTIME', 'MASKNAME', 'GRATING', 'CENTWAVE', 'OBSMODE',
               'GAIN', 'RDNOISE', 'MJD-OBS', 'PIXSCALE', 'DATE-OBS', 'AIRMASS']
        for jj in hed:
            hdr0.update(jj, hdr[jj])
        if 'south' in _tel.lower():
            _tel = 'gs'
        else:
            _tel = 'gn'
    ...
    if not dictionary['objname']:
        note = note + 'ERROR= OBJECT not defined '
    else:
        note = note + 'objname= ' + str(dictionary['objname']) + '\n'
    if not dictionary['ra0']:
        note = note + 'ERROR= RA not defined \n'
    else:
        note = note + 'ra= ' + str(dictionary['ra0']) + '\n'
    if not dictionary['dec0']:
        note = note + 'ERROR= DEC not defined \n'
    else:
        note = note + 'dec= ' + str(dictionary['dec0']) + '\n'
    if not dictionary['targid']:
        note = note + 'ERROR= TARGID not defined \n'
    else:
        note = note + 'targid= ' + str(dictionary['targid']) + '\n'
    if not dictionary['dateobs']:
        note = note + 'ERROR= DATE-OBS not defined \n'
    else:
        note = note + 'dateobs= ' + str(dictionary['dateobs']) + '\n'
    ...
    if 'ERROR' in note:
        ...
        if not agnkey.agnsqldef.getfromdataraw(agnkey.agnsqldef.conn, datarawtable, 'namefile',
                                               string.split(_output, '/')[-1], column2='namefile'):
            ...
            if _force == 'update':
                ...
                for voce in dictionary:
                    if voce != 'id' and voce != 'namefile':
                        agnkey.agnsqldef.updatevalue(datarawtable, voce, dictionary[voce],
                                                     string.split(_output, '/')[-1])
            elif _force == 'force':
