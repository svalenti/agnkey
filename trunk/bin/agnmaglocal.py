    ...
    for f in filterlist:
        ...
        if interactive and answ != 'q':  #  plots
            ...
            plt.figtext(.3, .4, 'std=%4.2f error=%5.3f' % \
                        (np.std(dmean[ijd]), np.std(dmean[ijd]) / np.sqrt(len(ijd))), fontsize=14)
            ...
            plt.ylim(min(rmean[ast]) - max(rstd[ast]) - .2, max(rmean[ast]) + max(rstd[ast]) + .2)
        ...
        for i in range(len(ljd)):
            ...
            if len(d1[0]) > 1 or len(d2[0]) > 1:
                t = lt[i]
                if t not in zerocor.keys():
                    zerocor[t] = {}
                if d not in zerocor[t].keys():
                    zerocor[t][d] = {}
...
if __name__ == "__main__":
    ...
    parser.add_option("-o", "--output", dest="output", default='', type="str", help='output name \t [%default]')
