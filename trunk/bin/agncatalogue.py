    ...
    if _exzp:
        ...
        for _filter2 in dicti2:
            ...
            for jj in  dicti2[_filter2][img2].keys():
                if 'ZP' in jj and _filter2 in dicti:
                    for img in dicti[_filter2].keys():
                        ...
                        agnkey.agnsqldef.updatevalue('dataredulco','zcol'+str(num),mm[1:],
                                                     string.split(re.sub('sn2.fits','fits',img),'/')[-1])
                        agnkey.agnsqldef.updatevalue('dataredulco','z'+str(num),result[1],
                                                     string.split(re.sub('sn2.fits','fits',img),'/')[-1])
                        agnkey.agnsqldef.updatevalue('dataredulco','c'+str(num),result[2],
                                                     string.split(re.sub('sn2.fits','fits',img),'/')[-1])
                        agnkey.agnsqldef.updatevalue('dataredulco','dz'+str(num),result[3],
                                                     string.split(re.sub('sn2.fits','fits',img),'/')[-1])
                        agnkey.agnsqldef.updatevalue('dataredulco','dc'+str(num),result[4],
                                                     string.split(re.sub('sn2.fits','fits',img),'/')[-1])
...
#   img 2  ###############
                ...
                if len(ww)>0:
                    mag0cut,mag1cut,dmag0cut,dmag1cut,ra0cut,dec0cut,racut,deccut = \
                        mag0cut[ww],mag1cut[ww],dmag0cut[ww],dmag1cut[ww],ra0cut[ww],dec0cut[ww],racut[ww],deccut[ww]
