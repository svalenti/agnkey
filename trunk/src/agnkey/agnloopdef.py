...
def run_getmag(imglist,_field,_output='',_interactive=False,_show=False,_bin=1e-10,magtype='mag', database='dataredulco'):
     import agnkey
     import datetime
     import agnsqldef
     ...
     elif _field=='apass': filters0=['B','V','gp','rp','ip','Bessell-B','Bessell-V','SDSS-R','SDSS-I','Pan-Starrs-G']
     ...
     for _tel in setup:
        for _fil in setup[_tel]:
            jd0=compress((array(filt)==_fil)&(array(tel)==_tel),array(jd))
            ...
            namefile0=compress((array(filt)==_fil)&(array(tel)==_tel),array(namefile))
                ...
                if i not in done:
                    ...
                    if len(ww)>=2:
                        ...
                        if magtype=='fit':
                             ...
                             mag1.append(mean(mag0[ww]))
                        try:
                             dmag1.append(std(mag0[ww])/sqrt(len(ww)))
                        except: 
                             dmag1.append(0.0)
            ...
            setup[_tel][_fil]['jd']=list(array(jd1)-.5)
     ...
     if _show:
          ...
          if _output:
               try:    plotfast(setup,_output)
               except: pass

     keytelescope={'1m0-03':'3','1m0-04':'4','1m0-05':'5','1m0-06':'6','1m0-07':'7','1m0-08':'8','1m0-09':'9','1m0-10':'10',\
     ...
     for _tel in setup:
        ...
        for  _fil in setup[_tel]:
            for j in range(0,len(setup[_tel][_fil]['jd'])):
                ...
                for filt in filters0:
                  if filt in filters:
                    if filt==_fil:   line=line+'%12.7s\t%12.6s\t' %(str(setup[_tel][_fil]['mag'][j]),str(setup[_tel][_fil]['dmag'][j]))
                    else:            line=line+'%12.7s\t%12.6s\t' %('9999','0.0')
...
#     if _output: ff.close()
     aaa=linetot.keys()
     if _output:      
          for gg in sort(aaa):   ff.write(linetot[gg]) 
     else:
          for gg in sort(aaa):   print linetot[gg]     
     if _output:      ff.close()

def run_cat(imglist,extlist,_interactive=False,mode=1,_type='fit',_fix=False, database='dataredulco',_field='slaon'):
     import agnkey
     direc=agnkey.__path__[0]
     ...
     for img in imglist:
          ...
          f.write(_dir+re.sub('fits','sn2.fits',img)+'\n')
     f.close()
     if _interactive:  ii= ' -i '
     else:             ii= ''
     if _fix: ff=' -c '
     else:    ff=''
     tt=' -t '+_type+' '
     if _field:  ss=' -s '+_field
     else: ss=''
...
#               ############################
               ...
               command='agnastro.py '+_dir+img+' '+rr+' '+ii+' -m  vizir --xshift '+str(_xshift)+' --yshift '+str(_yshift)+cc #+' '+ff+' '+cc+' -t '+_type+' '+ss
               print command
               os.system(command)
          elif status==0: print 'status '+str(status)+': WCS stage not done' 
          elif status==-1: print 'status '+str(status)+': sn2.fits file not found' 
...
def run_zero(imglist,_fix,_type,_field,_catalogue,_color='',interactive=False,redo=False,show=False,_cutmag=99,database='dataredulco',_calib=''):
     import agnkey
     import os,string,glob   #MySQLdb,
     direc=agnkey.__path__[0]
     for img in imglist:
          ...
          if _calib: ll= '--calib '+_calib
          else: ll=''
          if _color: hh=' -C '+_color+' '
          else: hh=''
          ...
          if status>=1:
               ...
               if not _catalogue:
                    ...
                    if _SN0:
                         if not _calib: 
                              ...
                              _catalogue=glob.glob(agnkey.__path__[0]+'/standard/cat/'+_field+'natural/*'+_SN0+'*')
               ...
               if _catalogue: cc=' -c '+_catalogue+' '
               else: cc=''
               command='agnabsphot.py '+_dir+re.sub('fits','sn2.fits',img)+' '+ii+rr+ff+cc+' -t '+_type+' '+ss+dd+hh+ll+' --cutmag '+str(_cutmag)
               print command
               os.system(command)
          elif status==0: print 'status '+str(status)+': WCS stage not done' 
          elif status==-1: print 'status '+str(status)+': sn2.fits file not found' 
...
def run_apmag(imglist,database='dataredulco'):
     import agnkey
     direc=agnkey.__path__[0]
     ...
     for img in imglist:
          ...
          if ggg:
               ...
               if os.path.isfile(_dir+img1):
                    command='agnnewcalib.py '+_dir+img1
                    print command
                    os.system(command)
               else:
...
def run_psf(imglist,treshold=5,interactive=False,_fwhm='',show=False,redo=False,xwindow='',database='dataredulco'):
     ...
     for img in imglist:
          ...
          if status>=1:
               ...
               command='agnpsf.py '+_dir+img0+' '+ii+' '+ss+' '+rr+' '+ff+' '+'-t '+str(treshold)+xwindow
               print command
               os.system(command)
          elif status==0: print 'status '+str(status)+': WCS stage not done' 
          elif status==-1: print 'status '+str(status)+': sn2.fits file not found' 
...
def run_fit(imglist,_ras='',_decs='',_xord=3,_yord=3,_bkg=4,_size=7,_recenter=False,_ref='',interactive=False,show=False,redo=False,dmax=51000,database= 'dataredulco'):
     import agnkey
     ...
     for img in imglist:
          ...
          if status>=1:
               ...
               command='agnsn.py '+_dir+img+' '+ii+' '+ss+' '+rr+' -x '+str(_xord)+' -y '+str(_yord)+' '+_ras+' '+_decs+' '+cc+' -b '+str(_bkg)+'  -z '+str(_size)+' --datamax '+str(dmax)
               ...
               print command
               os.system(command)
          elif status==0: print 'status '+str(status)+': psf stage not done' 
          elif status==-1: print 'status '+str(status)+': sn2.fits file not found' 
          elif status==-2: print 'status '+str(status)+': .fits file not found' 
          elif status==-4: print 'status '+str(status)+': bad quality image' 
          else: print 'status '+str(status)+': unknown status' 
...
def checkstage(img,stage,database='dataredulco'):
     ...
     if not ggg:     status=-3   # not in the redo table
          ...
          _dir=ggg[0]['wdirectory']
          if ggg[0]['quality']==1: status=-4   #  bad quality image
          else:
               if not os.path.isfile(_dir+img): status=-2  # fits image not in working dir
               elif not os.path.isfile(_dir+re.sub('fits','sn2.fits',img)): status=-1  # sn2 table not in working dir
     if stage=='wcs' and status>=-1:
          if ggg[0]['wcs']!=0:  status=1
          else: status=2
     elif stage=='psf' and status>=-1 and ggg[0]['wcs']==0:
          if ggg[0]['psf']=='X' :  status=1
          else: status=2
     elif stage=='psfmag' and status>=0 and ggg[0]['psf']!='X'  and ggg[0]['wcs']==0:
          if ggg[0]['psfmag']==9999 or ggg[0]['psfmag']==9999:  status=1
          else: status=2
     elif stage=='zcat' and status>=0 and ggg[0]['psf']!='X'  and ggg[0]['wcs']==0:
          if ggg[0]['zcat']=='X':  status=1
          else: status=2
     elif stage=='mag' and status>=0 and ggg[0]['zcat']!='X' and ggg[0]['psfmag']!=9999:
          if ggg[0]['mag']==9999:  status=1
          else: status=2
     elif stage=='abscat' and status>=0 and ggg[0]['zcat']!='X' and ggg[0]['psf']!='X' :
          if ggg[0]['abscat']=='X':  status=1   # mag should be replaced with 'cat'
          else: status=2
...
def getcoordfromref(img2,img1,_show,database='dataredulco'):   #img1.sn2  img2.sn2  import pyraf outside
    ...
    if psfx1!=None and psfy1!=None:
        ...
        aaa=iraf.wcsctran('STDIN','STDOUT',_dir1+img1,Stdin=[lll],inwcs='logical',units='degrees degrees',outwcs='world',columns='1 2',formats='%10.5f %10.5f',Stdout=1)[3]
        rasn1,decsn1=string.split(aaa)
        if _show:
            iraf.set(stdimage='imt8192')
            iraf.display(_dir1+re.sub('sn2.fits','fits',img1),1,fill=True,Stdout=1,zsc='yes',zra='yes')#,z1=0,z2=3000)
            iraf.tvmark(1,'STDIN',Stdin=[lll],mark="cross",number='no',label='no',radii=5,nxoffse=5,nyoffse=5,color=205,txsize=1)
...
#    raw_input('ddd')
    ...
    rracut=compress( (abs(array(ra02[pos2])-float(rasn1))<.05) & (abs(array(dec02[pos2])-float(decsn1))<.05),array(rra))
    ddeccut=compress( (abs(array(ra02[pos2])-float(rasn1))<.05) & (abs(array(dec02[pos2])-float(decsn1))<.05),array(ddec))
    ...
    if _show:  
        ...
        iraf.display(_dir2+re.sub('sn2.fits','fits',img2),2,fill=True,Stdout=1,zsc='yes',zra='yes')#,z1=0,z2=3000)
        lll=str(rasn2c)+' '+str(decsn2c)
        bbb=iraf.wcsctran('STDIN','STDOUT',_dir2+img2,Stdin=[lll],inwcs='world',units='degrees degrees',outwcs='logical',columns='1 2',formats='%10.5f %10.5f',Stdout=1)[3]
        iraf.tvmark(2,'STDIN',Stdin=[bbb],mark="cross",number='no',label='no',radii=5,nxoffse=5,nyoffse=5,color=206,txsize=1)
        from pylab import ion,plot
        ion()
        plot(rra,ddec,'or')
...
def filtralist(ll2,_filter,_id,_name,_ra,_dec,_bad,_filetype=1):
     ...
     if _filter:         #filter 
          if _filter=='sloan':
               ww=asarray([i for i in range(len(ll1['filter'])) if ((ll1['filter'][i] in ['zs','up','gp','ip','rp','SDSS-G','SDSS-R','SDSS-I','Pan-Starrs-Z']))])
          elif _filter=='landolt':
               ww=asarray([i for i in range(len(ll1['filter'])) if ((ll1['filter'][i] in ['U','B','V','R','I','Bessell-B','Bessell-V','Bessell-R','Bessell-I']))])
          elif _filter=='apass':
               ww=asarray([i for i in range(len(ll1['filter'])) if ((ll1['filter'][i] in ['B','V','Bessell-B','Bessell-V','gp','ip','rp','SDSS-G','SDSS-R','SDSS-I']))])
          elif _filter in ['zs','up','gp','ip','rp','U','B','V','R','I','SDSS-G','SDSS-R','SDSS-I','Pan-Starrs-Z','Bessell-B','Bessell-V','Bessell-R','Bessell-I']:
               ww=asarray([i for i in range(len(ll1['filter'])) if ((ll1['filter'][i] in [_filter]))])
          else:
              lista=[]
              for fil in _filter:
                  try: lista.append(agnkey.sites.filterst('lsc')[fil])
                  except:
                       try:
     ...
     if _bad:
          ...
          if _bad=='psfmag':   # do not consider standard field as bad psfmag files
               ww=asarray([i for i in range(len(ll1['objname'])) if ((ll1['objname'][i]) not in ['L104','L105','L95','L92','L106','L113','L101','L107','L110','MarkA','s82_00420020','s82_01030111','Ru152'])])
               if len(ww)>0:
                    for jj in ll1.keys(): ll1[jj]=array(ll1[jj])[ww]
               else:
                    for jj in ll1.keys(): ll1[jj]=[]
    ...
    if not ra1 and not dec1:    
        for img in imglist:
            ...
            if psfx!=None and psfy!=None:
                ...
                aaa=iraf.wcsctran('STDIN','STDOUT',img,Stdin=[lll],inwcs='logical',units='degrees degrees',outwcs='world',columns='1 2',formats='%10.5f %10.5f',Stdout=1)[3]
                try:
        ...
        for img in imglist:
            ...
            for i in dicti.keys():
                for j in dicti[i].keys():
                    ...
                    distvec,pos0,pos1=agnkey.agnastrodef.crossmatch(array([float(ra1)]),array([float(dec1)]),array(ra00,float),array(dec00,float),5)
                    if len(pos1)>=1:
...
#                        raw_input('ddd')
    if show:
        ...
        print median(dec)
        plot(((ra-median(ra))*3600)*cos(median(dec)*pi/180.),(dec-median(dec))*3600,'or',label='position')
    try:
...
def checkwcs(imglist,force=True,database='dataredulco',_z1='',_z2=''):
     ...
     for img in imglist:
          ...
          if status>=0 or force==False:
               ...
               if _SN0:
                    ccc=iraf.wcsctran('STDIN','STDOUT',_dir+img,Stdin=[str(_ra0)+' '+str(_dec0)],inwcs='world',units='degrees degrees',outwcs='logical',\
                                           columns='1 2',formats='%10.5f %10.5f',Stdout=1)
                    iraf.tvmark(1,'STDIN',Stdin=list(ccc),mark="circle",number='yes',label='no',radii=15,nxoffse=5,nyoffse=5,color=206,txsize=3)
               ...
               if len(_catalogue)>=1:
                    ...
                    aaa=iraf.wcsctran('STDIN','STDOUT',_dir+img,Stdin=list(bbb),inwcs='world',units='degrees degrees',outwcs='logical',columns='1 2',formats='%10.5f %10.5f',Stdout=1)
                    iraf.tvmark(1,'STDIN',Stdin=list(aaa),mark="cross",number='yes',label='no',radii=1,nxoffse=5,nyoffse=5,color=204,txsize=1)

               else:
                    catvec=agnkey.agnastrodef.querycatalogue('usnoa2',_dir+img,'vizir')
                    apix1=catvec['pix']
                    iraf.tvmark(1,'STDIN',Stdin=list(apix1),mark="circle",number='yes',label='no',radii=20,nxoffse=5,nyoffse=5,color=205,txsize=2)
     ...
     for img in imglist:
          ...
          if status>=0:   # or force==False:
               ...
               if _SN0:
                    ...
                    plt.plot([float(xPix)],[float(yPix)],marker='o',mfc='none',markersize=25,markeredgewidth=2, markeredgecolor = 'r', label=str(_SN0))
                    if _interactive:
                         plt.show()
                    else:
                         print _output
...
#    import  mysqldef #import updatevalue,getvaluefromarchive
    ...
    if 'wdirectory' in _dir[0]: _dir=_dir[0]['wdirectory']
    else:
         _dir=''
    if _dir: 
         if os.path.isfile(_dir+re.sub('.fits','.og.fits',_namefile[ii])) and os.path.isfile(_dir+re.sub('.fits','.rs.fits',_namefile[ii])):
              from pyraf import iraf
              iraf.digiphot(_doprint=0)
              iraf.daophot(_doprint=0)
    ...
    if event.key in ['d']:
         ...
         if _dir:
              agnkey.util.updateheader(_dir+re.sub('.fits','.sn2.fits',_namefile[ii]),0,{'PSFMAG1':[9999,'psf magnitude']})
              agnkey.util.updateheader(_dir+re.sub('.fits','.sn2.fits',_namefile[ii]),0,{'APMAG1':[9999,'ap magnitude']})
    elif event.key in ['u']:
    ...
    for _tel in _setup:
        ...
        for _fil in _setup[_tel]:
            shift=_shift[_fil]
            col=_color[_fil]
            plt.plot(array(_setup[_tel][_fil]['jd']),array(_setup[_tel][_fil]['mag'])+shift,_symbol[ii],color=col,markersize=5)
...
def plotfast(setup,output='',database='dataredulco'):#,band,color,fissa=''):
    ...
    for _tel in _setup:
        ...
        for _fil in _setup[_tel]:
            ...
            col=_color[_fil]
            print _tel,_fil
            jj=array(_setup[_tel][_fil]['jd']) #compress(array(_setup[_tel][_fil]['magtype'])>=1,array(_setup[_tel][_fil]['jd']))
            mm=array(_setup[_tel][_fil]['mag']) #compress(array(_setup[_tel][_fil]['magtype'])>=1,array(_setup[_tel][_fil]['mag']))
...
def chosecolor(allfilter,usegood=False,_field=''):
     goodcol={'B':'BV','V':'VR','R':'VR','I':'RI','U':'UB','u':'ug','g':'gr','r':'ri','i':'ri','z':'iz'}
     if not _field:
          if allfilter[0] in 'UBVRI':  tot,tot2='UBVRI','IRVBU'
          else:                        tot,tot2='ugriz','zirgu'
     else:
          if _field=='landolt':  tot,tot2='UBVRI','IRVBU'
          elif _field=='sloan':  tot,tot2='ugriz','zirgu'
          elif _field=='apass':  tot,tot2='BVgri','irgVB'
...
###########################################################################
def get_list(epoch,_telescope='all',_filter='',_bad='',_name='',_id='',_ra='',_dec='',database='dataredulco',filetype=1):
