...
#        CD2_1=(-1)*abs(CDELT0)*(abs(CDELT0)/CDELT0)*sin(theta)
 ...
 #       CDELT0=pixscale/3600.
    ...
    agnkey.util.updateheader(img,0,{'CTYPE1':[CTYPE1,''], 'CTYPE2':[CTYPE2,''], 'CRVAL1':[CRVAL1,''], 'CRVAL2':[CRVAL2,''],\
...
def querycatalogue(catalogue,img,method='iraf'):
        ...
        import agnkey
        from agnkey.util import delete
        from agnkey.util import readhdr,readkey3
        hdr=readhdr(img)
        _ra=readkey3(hdr,'RA')
        _dec=readkey3(hdr,'DEC')
        ...
        iraf.noao.astcat.catdb=agnkey.__path__[0]+'/standard/cat/catalogue.dat'
...
########
            ...
            stdcoo=agnkey.agnastrodef.vizq(_ra,_dec,catalogue,_size)
...
#        time.sleep(1)
        catvec=agnkey.agnastrodef.querycatalogue(catalogue,img,method)
        rmsx1,rmsy1,num1,fwhm1,ell1,ccc,bkg1,rasys1,decsys1=agnastrometry2([img],catalogue,_interactive,number1,sexvec,catvec,guess=False,fitgeo=_fitgeo,\
                                                                                 tollerance1=_tollerance1, tollerance2=_tollerance2,_update='yes',imex=_imex,nummin=_numin)
        if rmsx1>1 or rmsy1>1:
            catvec=agnkey.agnastrodef.querycatalogue(catalogue,img,method)
            rmsx2,rmsy2,num2,fwhm2,ell2,ccc,bkg2,rasys2,decsys2=agnastrometry2([img],catalogue,_interactive,number2,sexvec,catvec,guess=False,fitgeo=_fitgeo,\
                                                                                     tollerance1=_tollerance1, tollerance2=_tollerance2,_update='yes',imex=_imex,nummin=_numin)
            if rmsx2>1 or rmsy2>1:
                catvec=agnkey.agnastrodef.querycatalogue(catalogue,img,method)
                rmsx3,rmsy3,num3,fwhm3,ell3,ccc,bkg3,rasys3,decsys3=agnastrometry2([img],catalogue,_interactive,number3,sexvec,catvec,guess=False,fitgeo=_fitgeo,\
                                                                                         tollerance1=_tollerance1, tollerance2=_tollerance2,_update='yes',imex=_imex,nummin=_numin)
...
#################################################################################################
############################################################################################################
def agnastrometry2(lista,catalogue,_interactive,number,sexvec,catvec,guess=False,fitgeo='xyscale', tollerance1=100, tollerance2=30, _update='yes',imex=False,nummin=4):
    ...
    import time
    import datetime
    from agnkey.agnastrodef import wcsstart
    from agnkey.util import delete, readhdr,readkey3, display_image, defsex
    from pyraf import iraf
...
#####################################################################
    ...
    _CRPIX1=readkey3(hdr,'CRPIX1')
    _CRPIX2=readkey3(hdr,'CRPIX2')
    agnkey.util.updateheader(img,0,{'CRPIX1':[_CRPIX1-xoff,'']})
    agnkey.util.updateheader(img,0,{'CRPIX2':[_CRPIX2-yoff,'']})
...
def zeropoint(img,_field,verbose=False,catalogue=''):
    ...
    import math
    import agnkey
    from agnkey.util import delete,readhdr,readkey3
    from agnkey import sqlcl
    from pyraf import iraf
    iraf.noao(_doprint=0)
...
#    kk={'U':0.63,'u':0.70,'B':0.32,'g':0.26,'V':0.18,'r':0.15,'R':0.13,'i':0.08,'I':0.07,'z':0.06}
    if 1==1:
        if catalogue:
            stdcooC=agnkey.agnastrodef.readtxt(agnkey.__path__[0]+'/standard/cat/'+catalogue+'.cat')
            rastdC,decstdC=array(stdcooC['ra'],float),array(stdcooC['dec'],float)
            delete('tmp.stdC.pix')
            iraf.wcsctran(agnkey.__path__[0]+'/standard/cat/'+catalogue+'.cat','tmp.stdC.pix',img,inwcs='world',units='degrees degrees',outwcs='logical',\
                              columns='1 2',formats='%10.1f %10.1f',verbose='no')
            standardpixC=agnkey.agnastrodef.readtxt('tmp.stdC.pix')
...
#                elif _field=='sloan':      b,a,RR=median(zero),0,0
                elif _field=='sloan':      a, b, RR=agnkey.agnastrodef.linreg(colore, zero)
                xx=[min(array(colore)),max(array(colore))]
                yy=agnkey.agnastrodef.pval(array(xx),[b,a])
                result[filters[_filter]+col]=[a,b,RR]
            else: 
                agnkey.util.updateheader(img,0,{'PHOTZP':[zero[0],'MAG=-2.5*log(data)+PHOTZP']})
                b,a,RR=zero[0],0,0
                xx=colore
                yy=zero
                result[filters[_filter]+col]=[0,zero[0],0]                    
            if len(colore)>=1:
                if _field=='landolt':     agnkey.util.updateheader(img,0,{'PHOTSYS':['VEGA','mags (landolt catalogue)']})
                elif _field=='sloan':     agnkey.util.updateheader(img,0,{'PHOTSYS':['AB','mags (sloan catalogue)']})
...
def sloan2file(ra1, dec1, radius, magsel0, magsel1,_output):     #  ra in degree  and dec in degree
    ...
    _ids,_ras,_decs,_us,_gs,_rs,_is,_zs,_type,_dus,_dgs,_drs,_dis,_dzs=agnkey.agnastrodef.querysloan(ra1,dec1,float(radius),magsel0,magsel1)
