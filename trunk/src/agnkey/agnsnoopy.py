...
#########################################
    ...
    iraf.noao.digiphot.daophot.datapars.datamin = 0  # -100  #_datamin
...
#    iraf.noao.digiphot.daophot.centerpars.cbox = 0
    ...
    for i in range(len(tmptbl)):
        ...
        try:            apmag2.append(float(string.split(tmptbl[i])[1]))#-2.5*log10(_exptime))
        except:         apmag2.append(9999)
        try:            apmag3.append(float(string.split(tmptbl[i])[2]))#-2.5*log10(_exptime))
        except:         apmag3.append(9999)
    ...
    if _show:
        ...
        _tmp1,_tmp2,goon=agnkey.util.display_image('snfit.fits',1, z01, z02, False, _xcen=.25, _ycen=.75, _xsize=.3, _ysize=.3, _erase='no')
        agnkey.util.delete("tmptbl")
        ...
        ff.close()    
        iraf.tvmark(1,"tmptbl",autol='no',mark="none",inter='no',label='yes',txsize=2)
        _tmp1,_tmp2,goon=agnkey.util.display_image('skyfit.fits',1, z11, z22, False, _xcen=.75, _ycen=.25, _xsize=.3, _ysize=.3, _erase='no')
        s1 = 1
        s2 = -1*int(fwhm0)
        agnkey.util.delete("tmptbl")
        ff=open('tmptbl','w')
...
def manusn(img,imgpsf,dmag0,apori1,apori2,apori3,apmag1,apmag2,apmag3,fitmag,truemag,magerr,centx,centy,z11,z22,midpt,size,fwhm0,x1,y1,arterr):
   ...
   _tmp1,_tmp2,goon=agnkey.util.display_image('_snfit.fits',1, z01, z02, False, _xcen=.25, _ycen=.75, _xsize=.3, _ysize=.3, _erase='no')
   agnkey.util.delete("tmptbl")
...
########
        ...
        artap1,artap2,artap3,artmag1,artmag2,artmag3,artfitmag,arttruemag,artmagerr,artcentx,artcenty=fitsn(img,imgpsf,coordlist,_recenter,fwhm0,'reserr','artsn','artres',_show,_interactive,z11,z22,midpt,size,apco0,dmax)
        ...
        for ii in range(0,_numiter):
            ...
            artap1,artap2,artap3,artmag1,artmag2,artmag3,artfitmag,arttruemag,artmagerr,artcentx,artcenty=fitsn(img,imgpsf,coordlist,_recenter,fwhm0,'reserr','artsn','artres',_show,_interactive,z11,z22,midpt,size,0,dmax)
####### 
        if i==0: era='yes'
        else:    era='no'
        artx = .5+.25*artx
